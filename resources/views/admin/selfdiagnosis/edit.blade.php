...
<div class="d-flex flex-column-fluid">
    ...
    <div class="container">
        ...
        <div class="card card-custom gutter-b example example-compact">
            ...
            <div class="card-body">
                <form class="form" action="{{ route('category.update',$categorydata->id) }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PUT') }}
            <input required name="id" type="hidden" value="{{$categorydata->id}}" id="category_id">
                    <div class="card-body">
                        <div class="form-group row">
                            <div class="col-lg-6">
                                <label><span class="text-danger">*</span>&nbsp;Name :</label>
                                <input class="form-control  @if($errors->has('name')) is-invalid @endif maxlength" maxlength="255" type="text" name="name" id="name" placeholder="Enter name" value="{{ old('name',$categorydata->name) }}" />
                                @if($errors->has('name'))
                                ...
                                <select class="form-control groupofdocument @if($errors->has('groupofdocument')) is-invalid @endif" name="groupofdocument[]"  multiple="multiple">
                                    @if($groupofdocuments)
                                        @foreach($groupofdocuments as $group)
                                            <option value="{{$group->id}}" {{in_array($group->id,$Organizationtypegroupassigned) ? 'selected' : ''}}>{{$group->name}}</option>
                                        @endforeach
                                    @endif
                                </select>
                        ...
                        <div class="form-group row">
                            <div class="col-lg-8">
                                <label>Status :</label>
                                <div>
                                    <input name="status" data-switch="true" type="checkbox" data-on-color="success" data-off-color="warning" data-on-text="Active" data-off-text="Inactive" value="1" {{ $categorydata->status == 1 ? "checked" : "" }}>
                                </div>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function() {
    ...
    $('.groupofdocument').on('select2:unselecting', function (e) {
        var tagId = e.params.args.data.id; //your id
        var categoryId = $("#category_id").val();
        $.ajax({
            type:"post",
            url:"{{route('category.tagdelete')}}", 
            dataType: "json",
            data:{categoryId: categoryId, tagId: tagId, "_token": "{{ csrf_token() }}"},
